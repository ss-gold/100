...
const CONFIG = {
  // 두 번째 방 이미지가 반대로 보이면 아래 두 줄만 서로 바꾸면 됩니다.
...
const STORAGE_KEY = "ysi_escape_final_v1_inventory";
...
const maskStore = {};
const room2Visible = $("#room2Visible");
...
function handleMapClick(room, event) {
  ...
  const id = getNearestColor(room, r, g, b);
...
const handlers = {
  cabinet() {
    openModal({
      title: "수납장",
      text: "수납장을 열었다. 안쪽에는 책 몇 권과 스크랩북이 들어 있었다. 스크랩북에는 영화 티켓이 들어 있다. 영화 제목은…… '♠♠♤●●●○◇◇◇.' 같이 봤던 영화 티켓인데. 금성제가 스크랩을 해둔 건가?"
    });
  },
  ...
  book1() {
    openModal({
      title: "책장",
      text: "영화 대본집, 추리 소설, 모모, 그리고 문제집, 참고서들이 가득하다. 가장 밑 칸에 들어있는 건…… 미스터리 잡지인 '■■ □□□□'. 금성제가 가지고 있다고 말했던 책인 것 같다."
    });
  },
  ...
  book2() {
    openModal({
      title: "책장",
      text: "갖가지 책들이 엉망진창으로 꽂혀 있다. 표지도 제각각, 장르도 제각각. 이래서는 원하는 책을 한 번에 찾기 힘들지 않나? 심지어는 뒤집힌 책들도 있다. …… 어? 잠깐, 자세히 보니 책장 한 구역이 붉게 표시되어 있다. '책을 올바른 순서로 꽂아 책 제목의 첫 글자만 읽었을 때 ■■♠♠□□□□(이)가 되도록 맞춰라.' 라고 적혀 있다.",
      extra: makeAnswerBox("계간악마미스터리", () => {
        ...
        openModal({
          title: "책장",
          text: "올바른 책을 찾아 꽂으니 책장이 덜컹 움직인다. …… 아무래도 이 책장은 문이었던 모양이다. 책장이 끼이익 소리를 내며 묵직하게 열린다.",
  ...
  cat() {
    openModal({
      title: "고양이 인형",
      text: "금성제에게 줬던 고양이 인형……인 줄 알았는데 약간 다르다. 이 고양이 인형은 턱시도지만, 금성제에게 줬던 인형은, 그냥 검은 고양이였으니까. 그러니까, 그게, 어디에서 샀던 거더라. 가게 이름이 뭐였지?",
      extra: makeAnswerBox("말랑필로우", () => {
        openModal({
          title: "고양이 인형",
          text: "맞아. 그런 이름이었지. 아슬아슬한 높이 탓에 손을 겨우 뻗어 고양이 인형을 집었다. 고양이 인형의 배에는 '27831 1S 2B' 라고 적혀 있다. 숫자 야구 같네.",
  ...
  gramophone() {
    openModal({
      title: "축음기",
      text: "축음기처럼 생겼지만 자세히 보니 블루투스 스피커다. 유튜브와 연결된 듯한데……. 귀에 익은 노래가 흘러나온다. 이 노래는 분명, 그때 금성제한테 좋다고 했던 극아타의……"
    });
  },
  ...
  table() {
    openModal({
      title: "협탁",
      text: "축음기 아래의 협탁은 위, 아래 둘로 나뉘어 있다. 둘 다 잠겨 있는 것 같고……. 어느 쪽부터 풀어볼까?",
      choices: [
        {
          label: "위쪽 칸",
          onClick: () => openModal({
            ...
            extra: makeAnswerBox("면목중학교", () => {
              ...
              openModal({
                title: "협탁",
                text: "서랍이 열렸다. 신기하게 노래가 흘러나온다. 서랍 안에는 '97083 0S 4B'라고 적힌 메모가 들어 있었다. 숫자 야구 같네.",
        ...
        },
        {
          ...
          onClick: () => openModal({
            ...
            extra: makeAnswerBox("다시다시만나", () => {
              ...
              openModal({
                title: "협탁",
                text: "서랍이 열렸다. 신기하게 노래가 흘러나온다. 서랍 안에는 '79436 0S 2B'라고 적힌 메모가 들어 있었다. 숫자 야구 같네.",
  ...
  bed() {
    openModal({
      title: "침대",
      text: "푹신한 침대다. 매트리스가 좋은 건지 앉아 보아도 끼익거리는 소리 없이 부드럽다."
    });
  },
  ...
  piano() {
    openModal({
      title: "그랜드 피아노",
      text: "건반을 아무거나 눌러 보면 좋은 소리가 울린다. 제법 고급스러운 피아노인 것 같다.",
      choices: [{
        label: "연주하기",
        onClick: () => openModal({
          title: "그랜드 피아노",
          ...
          extra: makeAnswerBox("코끼리", () => {
            openModal({
              title: "그랜드 피아노",
              text: "코끼리를 전부 연주하자 앉아 있던 피아노 의자에서 달칵 소리가 난다. 일어나서 열어 보면, 안에는 작은 종이 쪽지가 들어 있었다. '54930 1S 2B'. 숫자 야구 같네.",
  ...
  chess() {
    openModal({
      title: "체스 판",
      text: "체스다. 금성제는 체스를 둘 줄 알까? 그런데 자세히 보니……. 체스판에 글자가 적혀 있다. '세 번째 찍어준 도장이 제목인 노래도 있대.'"
    });
  },
  ...
  closet() {
    openModal({
      title: "옷장",
      text: "옷장을 열었다. 안에는 교복이 여러 벌 들어있다. 벽산, 은장, 강학, 여일, 지성……? 뒤에 두 개는 왜 들어있는 거지?",
      choices: [
        ["byeoksan", "벽산 교복 챙기기"],
        ["eunjang", "은장 교복 챙기기"],
        ["ganghak", "강학 교복 챙기기"],
  ...
  chair2() {
    openModal({
      title: "의자",
      text: "의자다. 조금 무겁지만 이 방 안에서라면 옮길 수 있을 것 같다.",
      choices: [{
  ...
  window() {
    openModal({
      title: "창문",
      text: "걷힌 커튼 사이로 햇빛이 들어온다. 납치범이 가둔 방치고는 제법 따스하고 평화롭다. 어라? 창틀 위쪽에 뭔가가 있는 것 같은데……. 손이 안 닿아.",
      choices: [{
        label: "도구 사용하기",
        onClick: () => openModal({
          title: "인벤토리",
          ...
          extra: makeInventoryPicker((id) => {
            if (id === "chair") {
              openModal({
                title: "창문",
                text: "의자를 밟고 올라가 창틀 위에 있는 것을 집었다. 새콤달콤 울트라 키위 맛? 아래에 작은 쪽지가 깔려 있다. '65249 0S 1B' 숫자 야구처럼 보인다.",
...
function wearUniform(id, item) {
  ...
  const texts = {
    yeoil: "여일고 교복을 입은 모습이 거울에 비친다. 나백진이 입었을 땐 좀 더 차가워 보이는 이미지였는데……. 팔을 내리면 소매에 손이 가려지는 게, 사이즈도 조금 큰 것 같다. 누구 교복인 거지?",
    jiseong: "지성고 교복을 입은 모습이 거울에 비친다. 이런 교복이었던가? 사실 잘 기억은 안 난다. 조금 어색하네……",
    byeoksan: "벽산고 교복을 입은 모습이 거울에 비친다. …… 금성제가 보면 어떤 반응일까?",
    ganghak: "강학고 교복을 입은 모습이 거울에 비친다. 금성제가 자주 입던 호랑이 그림 티셔츠까지 입고 나니 뭔가, 굉장히 오묘한 느낌이 든다. 금성제는 어디 있는 거지?",
    eunjang: "은장고 교복을 입은 모습이 거울에 비친다. 익숙한 모습, 익숙한 냄새. 혹시 내 교복인가? 어라, 주머니에 뭔가가 들어있다. '23509 0S 3B' 라고 적힌 쪽지. 숫자 야구 같네."
  };
